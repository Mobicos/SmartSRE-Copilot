...
SmartSRE Copilot 当前仍处于开发阶段。除版本号、tag 和 release 制品外，2.0 生产能力已经按主路径实现：
LangGraph Decision Runtime、Agent 审批/恢复、轨迹回放、BFF 前端、Compose 生产路径、安全边界和质量门都在代码中具备。
上线前仍必须完成本地或 CI 的质量门、浏览器 E2E、Compose smoke、真实 Qwen key、生产密钥、CORS 和备份策略验证。
...
## Compose Profile 与 Smoke

默认 compose 栈启动应用主路径：PostgreSQL、Redis、MinIO、迁移任务、FastAPI 后端、worker 和 Next.js 前端。
可选 profile 用于上线形态验证：

- `gateway`：增加 Caddy，验证生产网关和反向代理路径。
- `observability`：增加 Prometheus、Loki 和 OpenTelemetry Collector。
- `vector-milvus`：在 pgvector 之外启用 Milvus/Attu 本地验证栈。
...
smoke 默认使用 `ENVIRONMENT=dev` 和 `AGENT_DECISION_PROVIDER=deterministic`，不依赖真实 Qwen key。
它会验证服务 healthcheck、迁移退出码、后端 `/health`、后端 `/metrics`、前端和 Caddy 网关。

如果 Docker 出现 `127.0.0.1:7890 refused` 这类代理错误，请先启动 Docker Desktop 配置的本机代理，
或清空 Docker Desktop 代理设置，并用 `docker pull pgvector/pgvector:pg16` 验证镜像拉取。镜像拉取恢复后再运行 smoke。
...
- 设置 `AGENT_DECISION_PROVIDER=qwen`，并配置真实 `DASHSCOPE_API_KEY`。
- PostgreSQL、Redis、MinIO 和 API key 必须使用唯一生产密钥，不能使用示例默认值。
- Prometheus 抓取后端 `/metrics`；OpenTelemetry tracing 仍按需独立启用。
- 不要把 `.env` 提交到 Git。
- 生产环境优先使用托管 PostgreSQL、Redis 和 Milvus/Zilliz。
- 上线前明确 PostgreSQL、对象存储和向量数据的备份恢复策略。
...
- `GET /api/agent/runs/{run_id}/replay`：查看 replay 摘要、指标、工具轨迹、审批和最终报告
- `GET /api/agent/runs/{run_id}/decision-state`：查看 observation、decision、evidence、handoff、recovery 和 resume 状态
- `GET /api/agent/approvals`：列出待审批和已处理审批
- `POST /api/agent/runs/{run_id}/approvals/{tool_name}`：批准或拒绝待审批工具调用
- `POST /api/agent/runs/{run_id}/approvals/{tool_name}/resume`：恢复已批准的 gated 工具调用
...
- 后端质量门通过：compile、Ruff lint/format check、mypy、Bandit、OpenAPI check 和完整 pytest。
- 前端质量门通过：`pnpm install --frozen-lockfile`、lint、typecheck、build 和 Playwright Agent E2E。
- Compose 配置校验和 `scripts\compose_smoke.ps1` 通过，gateway 与 observability profile 可用。
- 生产 `.env` 基于 `config/.env.prod.example`，使用 Qwen provider、真实 DashScope key、明确 CORS、API key 强制鉴权，以及唯一数据库、Redis、MinIO 密钥。
- Prometheus 能抓取 `/metrics`，如启用 tracing，OpenTelemetry 独立配置。
- PostgreSQL、对象存储和外部向量数据的备份恢复流程已经演练。
- 高风险或破坏性工具必须审批，且在工作台验证过 approval/resume 路径。
...
人类开发者请先阅读 `CONTRIBUTING.md`，其中包含分支、提交、PR、质量门、依赖、开发阶段锁定和合并规范。

AI coding agent 还应在改动前阅读 `AGENTS.md`。

开发阶段不要创建公开交付 tag、GitHub 交付制品、包分发或容器镜像分发自动化。
